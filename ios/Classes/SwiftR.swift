...
class SwiftR {
  static var connections = [SignalR1]()
...
open class SignalR1: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
  static var connections = [SignalR1]()
  ...
  open func send(_ data: Any?) {
    var json = "null"
    if let d = data {
      if let val = SignalR1.stringify(d) {
        json = val
      }
    }
...
open class Hub: NSObject {
  ...
  init(name: String, connection: SignalR1) {
    self.name = name
  ...
  open func invoke(_ method: String, arguments: [Any]? = nil, callback: ((_ result: Any?, _ error: Any?) -> ())? = nil) throws {
    ...
    if let args = arguments {
      for arg in args {
        if let val = SignalR1.stringify(arg) {
